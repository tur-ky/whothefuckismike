...
VMWrapper::VMWrapper() : hDLL(nullptr), isRunning(false), iVMR_Login(nullptr), iVMR_Logout(nullptr), iVMR_IsParametersDirty(nullptr), iVMR_GetParameterFloat(nullptr), iVMR_SetParameterFloat(nullptr), iVMR_GetLevel(nullptr) {
    BuildTrackedParams();
}
...
void VMWrapper::BuildTrackedParams() {
    ...
    std::vector<std::string> stripParams = {
        "Mute", "Solo", "Gain", "Color_x", "Color_y", "Comp", "Gate", 
        "Pan_x", "Pan_y", "EQGain1", "EQGain2", "EQGain3", 
...
bool VMWrapper::Initialize() {
    ...
    if (!hDLL) {
        hDLL = LoadLibraryA("VoicemeeterRemote64.dll"); // Fallback
...
void VMWrapper::SetParameter(const std::string& paramName, float value) {
    if (!isRunning || !iVMR_SetParameterFloat) return;
...
float VMWrapper::GetLevel(int nType, int nChannel) {
    if (!isRunning || !iVMR_GetLevel) return 0.0f;
    float val = 0.0f;
