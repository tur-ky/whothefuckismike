...
static bool RCP(const char*l,float*v){bool c=false;
char id[64];
snprintf(id,64,"##p_%s",l);
if(ImGui::IsItemHovered()&&ImGui::IsMouseClicked(1))ImGui::OpenPopup(id);
if(ImGui::BeginPopup(id)){if(ImGui::InputFloat("##v",v,0,0,"%.2f",ImGuiInputTextFlags_EnterReturnsTrue)){c=true;
...
bool ch=false;
    ...
    if(ImGui::IsItemHovered()&&ImGui::IsMouseDoubleClicked(0)){*v=0;
ch=true;
}
    if(RCP(label,v))ch=true;

    if(ImGui::IsItemActive()&&io.MouseDelta.y!=0){*v-=io.MouseDelta.y*(v_max-v_min)/150;
if(*v<v_min)*v=v_min;
if(*v>v_max)*v=v_max;
ch=true;
}
...
dl->PathStroke(IM_COL32(60,60,60,255),false,3.5f);
    ...
    if(t>.001f){dl->PathArcTo(cn,R-2,a0,av,40);
...
ImU32 sc=IM_COL32(160,160,160,255);
    ...
    if(*mode==0){dl->AddText(ImVec2(p.x+4,sy),IM_COL32(255,255,255,255),"VOICE");
ImVec2 cs=ImGui::CalcTextSize("Color Panel");
dl->AddText(ImVec2(p.x+sz-cs.x-4,sy),sc,"Color Panel");
}
    else if(*mode==1){dl->AddText(ImVec2(p.x+4,sy),IM_COL32(255,255,255,255),"MODULATION");
ImVec2 fs=ImGui::CalcTextSize("Fx Panel");
dl->AddText(ImVec2(p.x+sz-fs.x-4,sy),sc,"Fx Panel");
}
    else{dl->AddText(ImVec2(p.x+4,sy),IM_COL32(255,255,255,255),"POSITION");
...
dl->AddRect(gA,gB,IM_COL32(80,80,80,255));
    ...
    if(*mode==2){float aR=sz*.4f,aCy=gA.y+sz*.65f;
...
dl->AddText(ImVec2(gB.x-16,cy-6),sc,"R");
    ...
    else{int gd=(*mode==1)?4:2;
for(int i=1;
i<gd;
i++){dl->AddLine(ImVec2(gA.x+(sz/gd)*i,gA.y),ImVec2(gA.x+(sz/gd)*i,gB.y),gc);
...
dl->AddLine(ImVec2(gA.x,cy),ImVec2(gB.x,cy),IM_COL32(55,55,55,255));
    ...
    if(*mode==0){dl->AddText(ImVec2(gA.x+6,gB.y-16),sc,"Lo");
...
bool ch=false;
    ...
    if(ImGui::IsItemHovered()&&ImGui::IsMouseDoubleClicked(0)){*x=0;
*y=0;
ch=true;
s_DC=true;
}
    ...
    if(ImGui::IsItemActive()&&!s_DC){*x=(io.MousePos.x-cx)/(sz*.5f);
...
ch=true;
    ...
    ImVec2 np;
if(*mode==2)np=ImVec2(cx+(*x*sz*.45f),cy-(*y*sz*.45f));

    else{if(fabsf(*x)<.001f&&fabsf(*y)<.001f)np=ImVec2(cx,gB.y-6);
else np=ImVec2(cx+(*x*sz*.45f),cy-(*y*sz*.45f));
}
    if(np.x<gA.x+4)np.x=gA.x+4;
if(np.x>gB.x-4)np.x=gB.x-4;
if(np.y<gA.y+4)np.y=gA.y+4;
if(np.y>gB.y-4)np.y=gB.y-4;
...
s_PDC=true;
    ...
    if(ImGui::IsItemActive()&&!s_PDC){*x=(io.MousePos.x-cx)/(sz*.5f);
*y=-(io.MousePos.y-cy)/(sz*.5f);
if(*x<-1)*x=-1;
if(*x>1)*x=1;
if(*y<-1)*y=-1;
...
ch=true;
    ...
    if(ImGui::IsItemActive()&&io.MouseDelta.y!=0&&!ImGui::IsMouseDoubleClicked(0)){*v-=io.MouseDelta.y*(v_max-v_min)/height;
if(*v<v_min)*v=v_min;
if(*v>v_max)*v=v_max;
ch=true;
}
...
if(hY<p.y+hR)hY=p.y+hR;
if(hY>p.y+height-hR)hY=p.y+height-hR;
    ...
    // Bold dB text
    char buf[32];
if(fabsf(*v)<.01f)snprintf(buf,32,"0dB");
else snprintf(buf,32,fmt,*v);
...
ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding,3);
    ...
    if(ImGui::Button(label,ImVec2(48,20))){*v=!*v;
ch=true;
}
...
bool ch=false;
    ...
    if(ImGui::Button(id,ImVec2(48,20))){m=(m+1)%5;
*v=(float)m;
ch=true;
}
    ImGui::PopStyleColor(5);
...
int lit=(int)(pv*sg);
    ...
    for(int i=0;
i<sg;
i++){float sy=p.y+height-(i+1)*(sH+1);
