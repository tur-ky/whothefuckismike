...
struct ImageTexture{ID3D11ShaderResourceView*view=nullptr;
...
float GetVal(const std::string&n){if(!g_Locks[n])g_LocalCache[n]=g_VM.GetParameter(n);
return g_LocalCache[n];
}
void SetVal(const std::string&n,float v,bool a){g_LocalCache[n]=v;
g_VM.SetParameter(n,v);
g_Locks[n]=a;
}
static int GetLevelCh(const char*p){if(strcmp(p,"Strip[0]")==0)return 0;
if(strcmp(p,"Strip[3]")==0)return 6;
return 8;
}
...
static void DrawBtnCol(const char*vm,bool has_mc,bool is_input){
    ...
    for(int i=0;
i<5;
i++){ImGui::SetCursorPosY(y0+pos[i]);
bool r=GetVal(pre+"."+rt[i])>.5f;
if(UI::ToggleButton(rt[i],&r,true))SetVal(pre+"."+rt[i],r?1:0,false);
}
    ImGui::SetCursorPosY(y0+pos[5]);

    if(is_input){bool mono=GetVal(pre+".Mono")>.5f;
if(UI::ToggleButton("mono",&mono,false))SetVal(pre+".Mono",mono?1:0,false);
}
    else if(has_mc){bool mc=GetVal(pre+".MC")>.5f;
if(UI::ToggleButton("M.C",&mc,false))SetVal(pre+".MC",mc?1:0,false);
}
    else{float kv=GetVal(pre+".K");
if(UI::KButton("K",&kv))SetVal(pre+".K",kv,false);
}
    ImGui::SetCursorPosY(y0+pos[6]);
bool solo=GetVal(pre+".Solo")>.5f;
if(UI::ToggleButton("solo",&solo,false))SetVal(pre+".Solo",solo?1:0,false);

    ImGui::SetCursorPosY(y0+pos[7]);
bool mute=GetVal(pre+".Mute")>.5f;
if(UI::ToggleButton("Mute",&mute,false))SetVal(pre+".Mute",mute?1:0,false);
...
void DrawInputStrip(const char*vm){
    ...
    if(intellipan_mode[0]==1){tx=&fx_x;
ty=&fx_y;
}if(intellipan_mode[0]==2){tx=&px;
ty=&py;
}
    ImGui::SetCursorPosX((ImGui::GetWindowWidth()-130)*.5f);

    if(UI::Intellipan("IP##0",tx,ty,&intellipan_mode[0])){
        if(intellipan_mode[0]==0){SetVal(pre+".Color_x",*tx,ImGui::IsItemActive());
SetVal(pre+".Color_y",*ty,ImGui::IsItemActive());
}
        else if(intellipan_mode[0]==1){SetVal(pre+".fx_x",*tx,ImGui::IsItemActive());
SetVal(pre+".fx_y",*ty,ImGui::IsItemActive());
}
        else{SetVal(pre+".Pan_x",*tx,ImGui::IsItemActive());
SetVal(pre+".Pan_y",*ty,ImGui::IsItemActive());
    ...
    if(UI::CircularKnob("Comp",&comp,0,10))SetVal(pre+".Comp",comp,ImGui::IsItemActive());
else g_Locks[pre+".Comp"]=ImGui::IsItemActive();

    ImGui::SameLine(0,12);

    if(UI::CircularKnob("Gate",&gate,0,10))SetVal(pre+".Gate",gate,ImGui::IsItemActive());
else g_Locks[pre+".Gate"]=ImGui::IsItemActive();
...
float gain=GetVal(pre+".Gain");
    ...
    if(UI::CustomGainSlider("G##0",&gain,-60,12,"MIKE"))SetVal(pre+".Gain",gain,ImGui::IsItemActive());
else g_Locks[pre+".Gain"]=ImGui::IsItemActive();
...
ImGui::Text("EQUALIZER");
    ...
    if(UI::CircularKnob("Treble",&treble,-12,12))SetVal(pre+".EQGain3",treble,ImGui::IsItemActive());
else g_Locks[pre+".EQGain3"]=ImGui::IsItemActive();

    ImGui::SameLine(0,12);

    if(UI::CircularKnob("Bass",&bass,-12,12))SetVal(pre+".EQGain1",bass,ImGui::IsItemActive());
else g_Locks[pre+".EQGain1"]=ImGui::IsItemActive();
...
ImGui::SetCursorPosX((ImGui::GetWindowWidth()-panW)*.5f);
    ...
    if(UI::DrawPanBox("Pan",&panx,&pany)){SetVal(pre+".Pan_x",panx,ImGui::IsItemActive());
...
float gain=GetVal(pre+".Gain");
    ...
    if(UI::CustomGainSlider("G",&gain,-60,12,name))SetVal(pre+".Gain",gain,ImGui::IsItemActive());
else g_Locks[pre+".Gain"]=ImGui::IsItemActive();
...
ImGui::Spacing();
    ...
    if(!virt)DrawInputStrip(vm);
else DrawVirtualStrip(idx,name,vm,has_mc);
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE,LPSTR,int){
    ...
    if(!CreateDeviceD3D(g_hwnd)){CleanupDeviceD3D();
...
DWORD sz=sizeof(val);
        ...
        if(RegQueryValueExA(hKey,"WhoTheFuckIsMike",NULL,NULL,(LPBYTE)val,&sz)==ERROR_SUCCESS)isStartup=true;
RegCloseKey(hKey);
}
    bool done=false;
...
ImGui::SetNextWindowSize(io.DisplaySize);
        ...
        ImGui::SameLine(0,6);
if(ImGui::Button("_",ImVec2(20,22)))ShowWindow(g_hwnd,SW_MINIMIZE);

        ImGui::SameLine(0,2);
if(ImGui::Button("[]",ImVec2(20,22))){WINDOWPLACEMENT wp;
wp.length=sizeof(wp);
...
if(mp.y<30&&!ImGui::IsAnyItemHovered()&&ImGui::IsMouseClicked(0)){ReleaseCapture();
...
LRESULT WINAPI WndProc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam){
    ...
    switch(msg){
        case WM_NCCALCSIZE: if(wParam)return 0;
 break;
...
if(pt.x>rc.right-b)return HTRIGHT;
        ...
        case WM_SIZE:
            ...
            if(g_pd3dDevice&&wParam!=SIZE_MINIMIZED){CleanupRenderTarget();
...
void CleanupDeviceD3D(){CleanupRenderTarget();
...
void CreateRenderTarget(){ID3D11Texture2D*b;
